...
def divisons_and_their_size(sequence: object):
    result = {
        'sizes': [],
        'locations': []
    }
...
'''
def complited(sequence: object, values: str):
    ...
    for number in sequence:
        if number == -1:
            ...
            if negative_1 and positive_1:
                values_complited.append(couting)
                couting = 0
    ...
    for value in values_list:
        if value not in values_complited:
...
def working_with_divisions(COLUMNS, LINES, result):
    for column in range(COLUMNS):
        values_list = table[f'c{column}'].split(' ')
        sequence = list()

        for line in range(LINES):
            sequence.append(result[line][column])

        divisons = divisons_and_their_size(sequence)

    for line in range(LINES):
        values_list = table[f'l{line}'].split(' ')
        sequence = list()
